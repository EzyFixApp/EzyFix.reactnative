...
export default function CustomerResetPasswordScreen() {
  ...
  const handleResetPassword = async () => {
    ...
    // Debug logging
    if (__DEV__) {
      console.group('🔄 Reset Password Attempt');
      console.log('📧 Email:', email);
    ...
    try {
      ...
      // Try alternative API call approach
      try {
        const response = await authService.forgotPassword(resetPasswordData);
        
        if (__DEV__) {
          console.group('✅ Password Reset Response');
        ...
        if (__DEV__) {
          console.group('🔄 API Error - Trying direct fetch');
          console.error('AuthService error:', apiError);
          console.groupEnd();
        }

        // Try direct fetch as backup
        try {
          const directResponse = await fetch('https://ezyfix.up.railway.app/api/v1/auth/forgot-password', {
            method: 'POST',
            headers: {
          ...
          if (__DEV__) {
            console.group('🔄 Direct Fetch Result');
            console.log('Status:', directResponse.status);
            console.log('Response:', directResult);
            console.groupEnd();
          }
          ...
          throw apiError; // Use original error
        }
      }
      ...
      if (error.message?.includes('Invalid OTP') || error.reason?.includes('Invalid OTP')) {
        errorMessage = 'Mã OTP không hợp lệ hoặc đã hết hạn. Vui lòng yêu cầu mã mới.';
      } else if (error.message?.includes('status_code') && error.message?.includes('400')) {
        errorMessage = 'Thông tin không hợp lệ. Vui lòng kiểm tra lại email và mã OTP.';
